...
public class CommitLog {
    ...
    public void recoverNormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        if (!mappedFiles.isEmpty()) {
            //从倒数第三个文件开始进行恢复，不足3个，就从第一个（？8知道为啥）
            //感觉可能是因为前面的已经8需要了
    ...
    private boolean isMappedFileMatchedRecover(final MappedFile mappedFile) {
        ...
        if (this.defaultMessageStore.getMessageStoreConfig().isMessageIndexEnable()
            ...
            //文件的第一条消息的时间戳 < checkpoint里存的刷盘点 -> 这个文件部分是有效的，从这个文件开始恢复
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        //mappedFileQueue相当于commitLog存储的文件夹，mapperFile就是一个个commitLog文件
        MappedFile unlockMappedFile = null;
        MappedFile mappedFile = this.mappedFileQueue.getLastMappedFile();
        ...
        try {
            ...
            //如果mapperFile为空或者已经满了
            //创建一个
            if (null == mappedFile || mappedFile.isFull()) {
                mappedFile = this.mappedFileQueue.getLastMappedFile(0); // Mark: NewFile may be cause noise
            }
            //常见失败，返回CREATE_MAPEDFILE_FAILED
            if (null == mappedFile) {
                log.error("create mapped file1 error, topic: " + msg.getTopic() + " clientAddr: " + msg.getBornHostString());
                beginTimeInLock = 0;
                ...
                if (!this.requestsRead.isEmpty()) {
                    ...
                    for (GroupCommitRequest req : this.requestsRead) {
                        ...
                        for (int i = 0; i < 2 && !flushOK; i++) {
                            ...
                            if (!flushOK) {
                                //刷 底层是MappedByteBuffer.force()
                                CommitLog.this.mappedFileQueue.flush(0);
                            }
                        }

                        //设置request的future返回结果
